...
def ARDS_ventilator(patient, level_severity, VoT):
    ...
    data_mgr.set_results_filename("./test_results/XCOR/ARDS_ventilator{}_{}.csv".format(patient,
                                                                                                         level_severity))
    ...
    # Turns off spontaneous breathing
    dyspnea = SEDyspnea()
...
if __name__ == '__main__':
    ...
    #names = ["Jane"]#"Cynthia", "Gus", "Joel", "Nathan", "Rick", "Hassan", "Soldier", "Jeff", "Carol", "Jane"]
...
# Add a new thread for every patient at each severity, start each thread, and join them
    for i, name in enumerate(names):
        for severity in severities:
            processes.append(mp.Process(None, ARDS_ventilator, args=(name, severity, VT[i])))
